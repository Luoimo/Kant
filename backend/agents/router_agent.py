...
你的任务是在主流程开始前，对用户的提问进行意图分类和查询优化（Pre-processing）。

请将用户的输入分为以下两类之一：
1. "book_qa" : 询问书本知识、哲学概念、阅读建议等需要专业深读的问题。
2. "casual"  : 闲聊、打招呼等不需要查询书库的问题。

同时，如果是 "book_qa"，请提供一个针对向量检索优化的 `optimized_query`（例如补充哲学语境或提取核心概念）；如果是 "casual"，保持原样即可。

必须输出如下格式的纯 JSON（不要带代码块或多余文字）：
{
  "intent": "book_qa" 或 "casual",
  "optimized_query": "优化后的查询词"
}
"""
...
class RouterAgent:
    ...
    负责在进入主回答逻辑前，对 Query 进行意图识别和重写优化。
    """
